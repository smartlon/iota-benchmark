...
func StartLog(interval int, wg *sync.WaitGroup) {
    ...
    for _, v := range Interval_txs {
        if max_lat == 0 {
            max_lat = Transactions[v].Inherent_lat + Transactions[v].Confirm_lat
        }else if max_lat < (Transactions[v].Inherent_lat + Transactions[v].Confirm_lat){
            max_lat = Transactions[v].Inherent_lat + Transactions[v].Confirm_lat
        }
        if min_lat == 0 {
            min_lat = Transactions[v].Inherent_lat + Transactions[v].Confirm_lat
        } else if min_lat > (Transactions[v].Inherent_lat + Transactions[v].Confirm_lat){
            min_lat = Transactions[v].Inherent_lat + Transactions[v].Confirm_lat
        }
    ...
    fmt.Printf("[%d s - %d s]: Max Latency %f,\n", a, b, float64(max_lat)/1000.0)
    fmt.Printf("[%d s - %d s]: Min Latency %f,\n", a, b, float64(min_lat)/1000.0)
    fmt.Printf("[%d s - %d s]: Average Latency %f,\n", a, b, float64(totalLatency)/float64(total*1000))
